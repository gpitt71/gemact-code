# Supplementary material on the Data Generation Process

```{r, eval=TRUE}
packages <- c("data.table", "SynthETIC", "SPLICE", "tidyr", "reticulate", "ChainLadder")
...
# Computing the Chain Ladder ----

cl_reserve=MackChainLadder(incr2cum(clean.lt(incremental_payments_tr)),mse.method = "Mack",est.sigma = "Mack")
summary_cl <- summary(cl_reserve)

true_val_tr <- incr2cum(incremental_payments_tr)

diag = ChainLadder::getLatestCumulative(clean.lt(incr2cum(incremental_payments_tr)))

true_val <- sum(true_val_tr[,years])-sum(diag)

true_val_ay <- true_val_tr[,years]-diag
...
data.frame(ay=0:(years-1),
           uccl=(cl_reserve$FullTriangle[,years]-diag)/unit,
           msepcl=(cl_reserve$Mack.ProcessRisk[,years]+cl_reserve$Mack.ParameterRisk[,years])/unit,
           true_r=c(true_val_ay)/unit)  %>%
  xtable::xtable(label=NULL,digits=2) %>%
  rbind(c(NA,round(summary_cl$Totals[4,]/unit,2),round(summary_cl$Totals[5,]/unit,2),round(true_val/unit,2))) %>%
...
results <- matrix(ncol=years) 

for(trials in 1:100){
  print(trials)
  out0 <- claims_generator_mdn(random_state=trials,years=years)
...
true_val_tr <- incr2cum(incremental_payments_tr)
diag = ChainLadder::getLatestCumulative(clean.lt(true_val_tr))
true_val_ay <- unname(c(true_val_tr[,years]-diag)[1:years])
...
fwrite(results,
       file = 'simulated_reserves_sd.csv')
...
dt <- fread('simulated_reserves_sd.csv')[2:dim(dt)[1],]

round(apply(dt,FUN=sd,MARGIN=2)/unit,2)

round(sd(apply(dt,FUN=sum,MARGIN=1))/unit,2)

round(100*sd(apply(dt,FUN=sum,MARGIN=1))/mean(apply(dt,FUN=sum,MARGIN=1)),2)
