...
class LayerTower(list):
    ...
    def set_and_check_tower(self):
        ...
        hf.assert_equality(
            self[0].deductible, 0,
            logger=logger,
            name='First layer (retention) deductible is not 0'
            )
...
class Severity:
    ...
    def stop_loss_transformation(self, cover, deductible, size=50000, random_state=None):
        """
...
class LossModel:
        ...
        for i in range(self.policystructure.length):
            ...
            if self.aggr_loss_dist_method == 'mc':
                ...
                if self.aggr_loss_dist_method == 'recursion':
                    ...
                    aggr_dist_excl_aggr_cond = Calculator.discr_fourier_transform(
                        severity=sevdict,
                        discr_step=self.sev_discr_step,
                        tilt=self.tilt,
    ...
    def costing(self):
        ...
        :rtype: ``None``
        """
        if (self.__aggr_loss_dist is None):
        ...
        for idx in range(self.policystructure.length):
            ...
            aggr_loss_dist_excl_aggr_cond_ = self.aggr_loss_dist_excl_aggr_cond[idx]
            ...
            if layer.category in {'xlrs'}:
                premium *= self._reinstatements_costing_adjuster(
                    aggr_loss_dist=aggr_loss_dist_excl_aggr_cond_,
